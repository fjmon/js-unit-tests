...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
   // fail('Teste vazio!');
    ...
    // Teste se dentro do objeto retornado, a função retorna uma `key` com `value` igual à circunferência correta 
    // para um círculo de raio 2.
    // Teste se dentro do objeto retornado, a função retorna uma `key` com `value` igual à área correta 
    // para um círculo de raio 3.
    // Teste se a função retorna, em um objeto, os dados corretos de um círculo de raio 3.
    expect(circle(!'numero')).toBe(undefined)
    expect(circle(1)).toEqual({radius: 1, area: 3.14, circumference: 6.28});
    expect(Object.keys(circle(1)).length).toBe(3);
//    expect(circle(1)).toHaveProperty('radius', 'area', 'circumference').toBe(true);
...
// por que funcionou apenas comentando?
  });
});
